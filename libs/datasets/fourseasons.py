...
class FourSeasons(Dataset):
    """Base class of dataset loaders for FourSeasons Driving Sequence
    """

    def __init__(self, *args, **kwargs):
        self.time_offset = 0
        super(FourSeasons, self).__init__(*args, **kwargs)

        # undistortion model
        # camera_model_dir = os.path.join(self.cfg.directory.img_seq_dir, 'robotcar-dataset-sdk', 'models')
        camera_model_dir ="/home/marcelbp/Desktop/4Seasons-dataset/10-07/recording_2020-10-07_14-53-52/calib"
        img_dir = os.path.join(self.cfg.directory.img_seq_dir, self.cfg.seq, 'undistorted_images/cam0')
        # self.model = CameraModel(camera_model_dir, img_dir) # this is for evaluation, it seems not needed
    ...
    def synchronize_timestamps(self):
        ...
        # timestamps = np.loadtxt(timestamp_txt)[:, 0].astype(np.int) # originally. for use with stereo.timestamps
    ...
    def get_intrinsics_param(self):
        ...
        #                     )
        intrinsic_txt = "/home/marcelbp/Desktop/4Seasons-dataset/10-07/recording_2020-10-07_14-53-52/calib/undistorted_calib_0_ala_oxford.txt"
        ...
        # crop
        self.x_crop = [0.0, 1.0]
        self.y_crop = [0.0, 0.8] # same as oxford robotcar since height is 256 and not 320. width is 640 as is every other camera model in DF-VO
    ...
    def get_image(self, timestamp):
        ...
        img_path = os.path.join(self.data_dir['img'], 
                            "{:019d}.{}".format(timestamp, self.cfg.image.ext)  # This must be changed .... it's 19 digits for 4seasons and not 016d like oxford. same with CNN-SVO?
                            )
    ...
    def get_gt_poses(self):
        ...
        """
        # marcelprasetyo: not used. leave the config blank
